...
async function displayInformations(trusker) {
  ...
  const { isValid } = await inquirer.prompt([{ message: 'Les informations sont elles valides?', type: 'list', choices: ['Oui', 'Non'], name: 'isValid' }])
...
async function computeStage(stageIndex = 0, trusker = null) {
  ...
  try {
    ...
    try {
      ...
      return computeStage(stageIndex, trusker);
    }
  } catch (error) {
    console.log(error)
  }
